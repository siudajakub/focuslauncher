...
FocusLauncher supports a number of different languages, inherited from its upstream project
[Kvaesitso](https://github.com/MM2-0/Kvaesitso). Canonical English copy lives in
`core/i18n/src/main/res/values/strings.xml`; other locales live alongside it under
`core/i18n`. You can contribute translations by editing the relevant locale's
`strings.xml` and opening a pull request.

> [!NOTE]
> FocusLauncher does not yet run its own Weblate instance. Until one exists, submit
> translation changes directly as pull requests against the string resources.

The remainder of this page describes how the upstream string resources are organized, much of
which still applies to FocusLauncher's inherited strings.

There are two main components, `i18n` and `units`:
