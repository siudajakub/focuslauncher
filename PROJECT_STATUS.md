...
- Release blocker — keystore secret is currently INVALID: the nightly build fails at signing with `KeytoolException: Failed to read key ... Tag number over 30 is not supported`, i.e. the `KEYSTORE` repo secret is not a valid base64-encoded JKS (likely line-wrapped/corrupted). Re-encode single-line (`openssl base64 -A -in keystore.jks`) and reset `KEYSTORE` (and confirm `KEYSTORE_PASSWORD`/`SIGNING_KEY_ALIAS`/`SIGNING_KEY_PASSWORD`), then validate with a `release.yml` `workflow_dispatch` dry run.
